...
List<IdNameProj> findBestStudents(long nStudents);
 ...
 /***************************************************************************/
 @Query(value="select s.id as id, s.name as name from marks m join students s on m.student_id=s.id "
 		+ "join subjects sb on m.subject_id = sb.id where sb.subject_name = :subjectName group by id, name order by avg(m.mark) desc limit :nStudents",
 		nativeQuery = true)
...
@Query(value="select sb.id as id, sb.subject_name as name from marks m  "
		...
		+ "group by id, name order by avg(m.mark) desc limit 1",
nativeQuery=true)
...
/***********************************************************************************/
@Query ("select sb.id as id, sb.subjectName as name from MarkEntity m right join m.subject sb"
		+ " group by id, name having coalesce(avg(m.mark), 0) < :avgMark")
List<IdNameProj> findSubjectsAvgMarkLess(double avgMark);
}
